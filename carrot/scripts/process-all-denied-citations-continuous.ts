/**
 * Process ALL Denied Citations Continuously
 * 
 * Reprocesses all denied citations and processes them until complete
 * Automatically triggers agent feeding and hero image generation for saved items
 * 
 * Usage:
 *   ts-node scripts/process-all-denied-citations-continuous.ts --patch=israel --batch-size=10
 */
...
async function processAllDeniedCitationsContinuous(
  patchHandle: string,
  batchSize: number = 10,
  pauseBetweenBatches: number = 2000
) {
  console.log(`\n🚀 Processing ALL denied citations continuously for patch: ${patchHandle}\n`)
  ...
  console.log(`   Ready to process: ${unprocessedCount}`)
  console.log()

  // First, reset all denied citations
  console.log('🔄 Resetting all denied citations for reprocessing...\n')
  ...
  console.log(`✅ Reset ${resetResult.count} denied citations\n`)
  ...
  while (true) {
    ...
    try {
      ...
      if (processed % batchSize === 0) {
        ...
        console.log(`\n📊 Progress: ${processed} processed (${saved} saved, ${denied} denied, ${failed} failed)`)
        console.log(`   Batch time: ${batchTime}s (${rate} citations/s)`)
        console.log(`   Total time: ${elapsed}s`)
        console.log(`   Remaining: ~${remaining}`)
        console.log(`   Save rate: ${((saved / processed) * 100).toFixed(1)}%\n`)
...
async function main() {
  try {
    ...
    await processAllDeniedCitationsContinuous(patchHandle, batchSize, pauseBetweenBatches)
  } catch (error) {
