/**
 * Enrich content with quotes and grammar cleanup using DeepSeek directly
 */
...
async function enrichContent() {
  ...
  // Get all content items that need enrichment
    ...
    try {
      ...
      if (item.summary || (item.facts && Array.isArray(item.facts) && item.facts.length > 0)) {
        ...
        if (cleanupResult.summary && cleanupResult.summary.length > 0) {
          ...
          console.log(`   ✅ Summary cleaned (${cleanupResult.summary.length} chars)`)
        }
        ...
        if (cleanupResult.keyFacts && cleanupResult.keyFacts.length > 0) {
          ...
          console.log(`   ✅ Key facts cleaned (${cleanupResult.keyFacts.length} facts)`)
        }
